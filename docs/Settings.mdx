...
To change the prefix to anything you like, follow the steps below:
  ...
  - Use the command `!settings prefix <NewPrefix>` (Note: Replace `<NewPrefix>` with the prefix you would like to set and then use the new prefix when executing commands)
